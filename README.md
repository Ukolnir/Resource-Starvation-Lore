...
> [!note]
> Здесь только то, что могут знать персонажи. Наружу выкладываются лишь заметки с пометкой `publish: true`; мастерские материалы — тайны мира, статы, спойлеры — остаются в приватном репозитории и сюда не попадают.

> [!important]
> Каталог `content/` — **производный**. Он целиком переписывается скриптом `publish.mjs` из приватного мастерского репозитория, где живёт весь лор. Правки здесь будут затёрты при следующей выкладке: вносить их нужно в мастерском репозитории.

## Что внутри

| | |
| --- | --- |
| `content/` | лор в Markdown, разложенный из мастерского репозитория |
